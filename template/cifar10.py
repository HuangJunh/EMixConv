...
class TrainModel(object):
    def __init__(self, is_test, batch_size, weight_decay):
        if is_test:
            full_trainloader = data_loader.get_train_loader('../datasets/CIFAR10_data', batch_size=batch_size, augment=True,shuffle=True, random_seed=1000, show_sample=False,num_workers=4, pin_memory=True)
            ...
            trainloader, validate_loader = data_loader.get_train_valid_loader('../datasets/CIFAR10_data', batch_size=256, augment=True, subset_size=1, valid_size=0.1, shuffle=True,random_seed=1000, show_sample=False,num_workers=4, pin_memory=True)
        ...
        if not is_test:
            self.supernet_path = './trained_models/supernet_cifar10.pt'
    ...
    def test(self,p):
        ...
        for _, data in enumerate(self.testloader, 0):
            ...
            correct += (predicted == labels.data).sum()
        if correct / total > self.best_acc:
            torch.save(self.net.state_dict(), './trained_models/best_CNN_c10.pt')
            self.best_acc = correct / total
